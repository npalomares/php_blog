...
?>
  	<article class="post">
          ...
          <h2 class="post"><a href="index.php?page=single&amp;post_id=<?php echo $row_latest['post_id'];?>">
          <?php echo $row_latest['title']; ?></a></h2>
          
          <div class="meta"><span class="date"><?php echo convert_date($row_latest['date']); ?></span> in category <a href="index.php?page=category&amp;category_id=<?php 
                echo $row_latest['category_id']; ?>"><?php echo $row_latest['name'] ?></a></div><!--END meta-->

 <div class="entry"><?php echo $row_latest['body']; ?></div><!--END ENTRY-->

  <hr />
 </article> <!--END POST--><?php } //end --while-- loop ?> 
